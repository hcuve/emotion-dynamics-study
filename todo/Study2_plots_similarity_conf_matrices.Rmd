...
confusion_byds_vt_full_coragg%>%
  ...
  select(cor_morph_orig),

confusion_byds_vt_full_coragg%>%
  subset(DATASET != "ADFES")%>%
  select(cor_morph_orig),by= "Participant")%>%
  ggplot(aes(cor_morph_orig.x, cor_morph_orig.y))+
  geom_point()+
...
```{r}
View(confusion_byds_vt_full_cor_nodiagnonal_agg1)
...
confusion_byds_vt_full_cor_nodiagnonal_agg%>%
  subset(DATASET == "ADFES")%>%
  select(cor_morph_orig_z),

confusion_byds_vt_full_cor_nodiagnonal_agg%>%
  subset(DATASET != "ADFES")%>%
  select(cor_morph_orig_z),by= "Participant")%>%
  ggplot(aes(cor_morph_orig_z.x, cor_morph_orig_z.y))+
  geom_point()+
  geom_smooth(method = "lm")+
  ggpubr::stat_cor()
```
