...
db_of7_new %>%
  ...
  geom_smooth(aes(group = Emotion), se = T)+
  facet_grid(Dataset~morph)
...
db_of7_new%>%
  ggplot(aes(timebin, face_action_AVG, color = morph, group = video))+
  stat_smooth(se = F)+
  facet_grid(~Dataset)
...
db_of7_new_2021<- db_of7_new$face_action_AVG2 %>%
  group_by(Emotion,Dataset, morph,video, )%>%
  arrange( Emotion,Dataset, morph,video,timebin)%>%
  mutate(face_action_AVG2 = face_action_AVG- mean(face_action_AVG[1:4], na.rm = TRUE))%>%
